...
# This script handles the complete UI testing workflow:
...
cleanup_test_results() {
    ...
    if [ -d "playwright" ]; then
        ...
        print_success "  playwright directory cleaned"
    fi
    ...
    if [ -d "ui-tests" ]; then
        ...
        print_success "  ui-tests directory cleaned"
    fi
...
# Main execution
main() {
    echo -e "${BLUE}🚀 Starting comprehensive UI test workflow...${NC}"
    echo ""
    ...
    # Step 5: Run UI tests
    print_status "Step 5: Running UI tests..."
    echo ""
